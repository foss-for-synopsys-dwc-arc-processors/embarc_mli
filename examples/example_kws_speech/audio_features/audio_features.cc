...
fbank_features::info fbank_features::get_info(const params& config) {
    ...
    const uint32_t dyn_mem_sz = sizeof(rfft_f32_t) + 2 * (config.num_fbank_bins * sizeof(int)) 
...
fbank_features* fbank_features::create_fbank_fex_module(const params& config, float* state_fastbuf_a,
    ...
    if(is_valid) {
        ...
        out->window_func_fast_buf_a_ = (__XY float*)state_fastbuf_a;
    ...
    // Create filter banks 
    if(is_valid) {
        out->mel_filters_storage_fast_buf_a_ = (__XY float*)&state_fastbuf_a[state_fastbuf_occupied];
    ...
    if(is_valid) {
        out->rfft_ = new rfft_wrapper((__XY float*)&state_fastbuf_a[state_fastbuf_occupied], out->frame_len_padded_);
...
int fbank_features::compute(const int16_t* samples, float* out_features, float* temp_fast_buf_a, float* temp_fast_buf_b) {
    ...
    values = apply_window((__XY float*)temp_fast_buf_b, window_func_fast_buf_a_, in_frame_len,
...
//====================================================
float au_fex::mel_to_freq(const float mel_freq) {
    return 700.0f * (expf(mel_freq / 1127.0f) - 1.0f);
}
float au_fex::freq_to_mel(const float freq) {
...
//====================================================
int au_fex::transform_normalize(const __XY int16_t* __restrict in, const int vals, const float norm_scale, 
...
//====================================================
int au_fex::power_spect(const __XY float* __restrict in_spect, const int spect_bins, 
...
//====================================================
int au_fex::ampl_spect(const __XY float* __restrict in_spect, const int spect_bins,
...
//====================================================
int au_fex::create_hann_window(const int vals, float*  out_window) {
    int idx = 0;
...
//====================================================
int au_fex::apply_window(const __XY float* __restrict in, const __XY float* __restrict window,
...
//====================================================
int au_fex::create_mel_triangle_filters(const int filters_num, const int sample_rate, const int freq_bins,
...
//==================================================================================
int au_fex::mel_triangle_filters_buf_length(const int filters_num, const int sample_rate,
...
//====================================================
int au_fex::apply_filter_banks(const __XY float* __restrict in, float** filters, 
...
//====================================================
int au_fex::log_norm(const __XY float* __restrict in, const int vals, __XY float* __restrict out) {
    int idx = 0;
    for (; idx < vals; idx++) {
        out[idx] = logf(in[idx]);
    }
...
//====================================================
int au_fex::matrix_vector_mul(const __XY float* __restrict in_matrix, const __XY float* __restrict in_vector,
