...
#------------------------------------------------------------
func_body_template_file = "mli_krn_depthwise_conv2d_func_body.txt"
...
stride = 1
f_list.extend([Func(fbase, 1, 2, 1, stride, stride, corefunc, "krnpad")]) #k_width == 1
f_list.extend([Func(fbase, 2, 1, 1, stride, stride, corefunc, "krnpad")]) #k_heigth == 1

#stride = 1, channels == 1
corefunc = "conv2d_chw_str1"
stride = 1
kernel_range = range(2,8)#range(1,6)
f_list.extend([Func(fbase, k, k, 1, stride, stride, corefunc, "krnpad") for k in kernel_range ]) #channels == 1
...
k = 0
ch = 0
f_list.extend([Func(fbase, k, k, ch, stride, stride, corefunc, "krnpad")])

#stride = 1, fix dimension
corefunc = "conv2d_chw_str1"
stride = 1
f_list.extend([Func(fbase, 1, 2, 1, stride, stride, corefunc, "nopad")]) #k_width == 1
f_list.extend([Func(fbase, 2, 1, 1, stride, stride, corefunc, "nopad")]) #k_heigth == 1

#stride = 1, channels == 1
corefunc = "conv2d_chw_str1"
stride = 1
kernel_range = range(2,8)#range(1,6)
f_list.extend([Func(fbase, k, k, 1, stride, stride, corefunc, "nopad") for k in kernel_range ]) #channels == 1
