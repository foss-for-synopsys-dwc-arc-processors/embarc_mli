...
#pragma clang loop unroll_count(2)
    for (idx = 0; idx < vals - (ch_unroll - 1); idx+=ch_unroll) {
        int32_t multi_in0 = *(int32_t*)in;
        int32_t multi_in1 = *(int32_t*)(in + in_unroll_step);
        int32_t multi_in2 = *(int32_t*)(in + 2 * in_unroll_step);
        int32_t multi_in3 = *(int32_t*)(in + 3 * in_unroll_step);
#pragma clang loop unroll(full)
        for (int i = 0; i < ch_unroll; i++) {
            accu.accu0 = mli_prv_mac_load_v_s(accu.accu0, krn, (io_T)multi_in0);
            if (unroll > 1) {
                accu.accu1 = mli_prv_mac_load_v_s(accu.accu1, krn, (io_T)multi_in1);
            }
            if (unroll > 2) {
                accu.accu2 = mli_prv_mac_load_v_s(accu.accu2, krn, (io_T)multi_in2);
            }
            if (unroll > 3) {
                accu.accu3 = mli_prv_mac_load_v_s(accu.accu3, krn, (io_T)multi_in3);
            }

            krn += krn_step;
            multi_in0 = multi_in0 >> (sizeof(io_T) * 8);
...
static MLI_FORCE_INLINE grpacc_T dotprod3D_v_pad_gather1_unroll (
    ...
    const vNx2int_t vindex = to_vNx2int_t(make_vindex(ext_width, height, in_col_step, in_row_step));
...
#pragma clang diagnostic ignored "-Wpass-failed"
    for (int ch = 0; ch < channels; ch++) {
        // gather load w x h samples from input
        auto in_gather = mli_prv_gather_load_nx2_samples(in, vindex, ext_width * height);
#pragma clang loop unroll(full)
        for (int row = 0; row < height; row++) {
#pragma clang loop unroll(full)
            for (int clmn = 0; clmn < width; clmn++) {
                ...
                if (unroll > 1) {
                    in_T input = in_gather[row * ext_width + clmn + 1];
                    accu.accu1 = mli_prv_mac_load_v_s(accu.accu1, krn, input);
                }
                if (unroll > 2) {
                    in_T input = in_gather[row * ext_width + clmn + 2];
                    accu.accu2 = mli_prv_mac_load_v_s(accu.accu2, krn, input);
                }
                if (unroll > 3) {
                    in_T input = in_gather[row * ext_width + clmn + 3];
