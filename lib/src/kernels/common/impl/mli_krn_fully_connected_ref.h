...
//========================================================
template <typename i_T, typename w_T, typename o_T, typename b_T, typename acc_T, typename quant_T, bool no_zp>
MLI_FORCE_INLINE void inner_product(
        const MLI_PTR(i_T) __restrict in,
        const MLI_PTR(w_T)  __restrict weights,
        const MLI_PTR(b_T)  __restrict biases,
        MLI_CONV_OUT_PTR(o_T) __restrict out,
    ...
    if (has_bias) {
        acc_T other_additives = mli_math_mul_fx<i_T, acc_T>(0, 0);
        ...
        acc_T in_additives = mli_math_mul_fx<i_T, acc_T>(0, 0);
        in_additives  = mli::krn::in_additive(in, in_additives, &quant_params, in_elements, 1, 1, 1);
        for (int o_idx = 0; o_idx < out_elements; o_idx++) {
            mli::krn::adjust_quant_params(&quant_params, o_idx);
            acc_T accu = mli_math_mul_fx<i_T, acc_T>(0, 0);
            accu = dotprod1D(in, &weights[o_idx], accu, in_elements,
                            1, w_ch_out_mem_stride);
            accu = mli_math_add_fx(accu, in_additives);
            // Cast result to output type with scaling
            o_T out_val = mli::krn::result_cast<o_T, acc_T, quant_T>(accu, &quant_params);
            out[o_idx] = out_val;
        }
...
//========================================================================================
template <typename i_T, typename w_T, typename o_T, typename b_T, typename acc_T, typename quant_T, bool is_bias_ext>
MLI_FORCE_INLINE void fully_connected_prepare_and_run(
    ...
    const MLI_PTR(i_T) in_ptr = mli_prv_tensor_data_ptr<MLI_PTR(i_T)>(in);
    const MLI_PTR(w_T) w_ptr = mli_prv_tensor_data_ptr<MLI_PTR(w_T)>(weights);
    MLI_CONV_OUT_PTR(o_T) out_ptr = mli_prv_tensor_data_ptr<MLI_CONV_OUT_PTR(o_T)>(out);
    ...
    const MLI_PTR(b_T) b_ptr  = nullptr;
    if (has_bias) {
        b_ptr  = mli_prv_tensor_data_ptr<MLI_PTR(b_T)>(bias);
    }
    ...
    //=======================================================================
    mli::krn::inner_product<i_T, w_T, o_T, b_T, acc_T, quant_T, is_bias_ext>(
            in_ptr, w_ptr, b_ptr, out_ptr, in_sz, ch_out, w_ch_out_mem_stride, /* cent_area, */ params, (o_T)val_limit.min, (o_T)val_limit.max);
}
