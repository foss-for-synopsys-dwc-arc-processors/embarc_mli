...
MLI_FORCE_INLINE void gru_cell_prepare_and_run(
    ...
    for (int timestep = 0; timestep < seq_len; timestep++) {
        ...
        mli::krn::eltwise_prepare_and_run<io_T, io_T, ELTWISE_MUL, /*convert*/ asym, /*no_scalar*/ true, /*no_out_update*/ true, /*shape_1d*/ true>(&tmp_res_upd_gate, &current_hidden, &prev_out_reset);
        tmp_res_upd_gate.data = dtcntr_update_gate; // switch data ptr to update_gate
        mli::krn::eltwise_prepare_and_run<io_T, io_T, ELTWISE_MUL, /*convert*/ asym, /*no_scalar*/ true, /*no_out_update*/ true, /*shape_1d*/ true>(&current_hidden, &tmp_res_upd_gate, &current_out);
        mli::krn::eltwise_prepare_and_run<io_T, io_T, ELTWISE_SUB, /*convert*/ asym, /*no_scalar*/ false, /*no_out_update*/ true, /*shape_1d*/ true>(&one, &tmp_res_upd_gate, &tmp_res_upd_gate);
        ...
        //tmp_res_upd_gate.data = dtcntr_update_gate; // switch data to update_gate
        mli::krn::eltwise_prepare_and_run<io_T, io_T, ELTWISE_MUL, /*convert*/ asym, /*no_scalar*/ true, /*no_out_update*/ true, /*shape_1d*/ true>(&new_gate, &tmp_res_upd_gate, &temp);
        ...
        if (cfg->results == RNN_OUT_ALL) {
            mli_prv_tensor_set_data_ptr<io_T>(&rnn_out,
                                              mli_prv_tensor_data_ptr<io_T *>(&rnn_out) + gru_out_elements);
        }
    ...
    out->mem_stride[0] = gru_out_elements;
    out->mem_stride[1] = 1;
}
