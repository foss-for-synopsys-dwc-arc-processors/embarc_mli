...
Rescale::Rescale(void* kernel_private_data_buffer, size_t size,
    ...
    m_enc_param = private_buffer.enc_param;
    m_output = private_buffer.output;

    m_tile_metadata.rescale_axis = private_buffer.rescale_axis;
    if (m_tile_metadata.rescale_axis == kPerTensorQuantDim) m_tile_param_max_size = 1;
    else {
      const auto& enc_cfg = m_enc_param.get_config();
      m_tile_param_max_size = MAX(enc_cfg.get_first_size(m_tile_metadata.rescale_axis),
                                  enc_cfg.get_size(m_tile_metadata.rescale_axis));
    }
    ...
    else if (input.get_elem_size() == sizeof(int8_t)) {
      m_tile_metadata.input.data.mem.pi8 = input_internal.get_ptr<int8_t>();
      m_tile_metadata.input.el_type = MLI_EL_SA_8;
    }
    else {
        MLI_ASSERT(0);
    }
    ...
    if (output.get_elem_size() == sizeof(int8_t)) {
        ...
        m_tile_metadata.output.el_type = MLI_EL_SA_8;
    } 
    else if (output.get_elem_size() == sizeof(int32_t)) {
        m_tile_metadata.output.data.mem.pi32 = output_internal.get_ptr<int32_t>();
        m_tile_metadata.output.el_type = MLI_EL_SA_32;
    }
    else {
        MLI_ASSERT(0);
    }
    ...
    // set in_bias
    InternalBuffer encoded_params_internal(private_buffer.enc_param.get_buf(), membases, num_mems);
    if (input.get_elem_size() == sizeof(int32_t)) {
        encoded_params_internal.set_elem_size(sizeof(int32_t));
        if(m_tile_metadata.rescale_axis == kPerTensorQuantDim) {
      ...
      if (m_tile_metadata.rescale_axis == kPerTensorQuantDim) {
        m_tile_metadata.in_bias.rank = 0;
        m_tile_metadata.in_bias.data.mem.i8 = encoded_params_internal.read<int8_t>(0);
      }
      else { // per-axis
        m_tile_metadata.in_bias.rank = 1;
        m_tile_metadata.in_bias.data.mem.pi8 = encoded_params_internal.get_ptr<int8_t>();
      }
      encoded_params_internal.inc(m_tile_param_max_size);
      m_tile_metadata.in_bias.el_type = MLI_EL_SA_32;
    }
    ...
    if (output.get_elem_size() == sizeof(int8_t)) {
        ...
        m_tile_metadata.out_bias.el_type = MLI_EL_SA_8;
    }
    else if (output.get_elem_size() == sizeof(int32_t)) {
      encoded_params_internal.set_elem_size(sizeof(int32_t));
      if (m_tile_metadata.rescale_axis == kPerTensorQuantDim) {
        m_tile_metadata.out_bias.rank = 0;
        m_tile_metadata.out_bias.data.mem.i32 = encoded_params_internal.read<int32_t>(0);
      }
      else {
        m_tile_metadata.out_bias.rank = 1;
        m_tile_metadata.out_bias.data.mem.pi32 = encoded_params_internal.get_ptr<int32_t>();
      }
      m_tile_metadata.out_bias.el_type = MLI_EL_SA_8;
    }
    else {
    ...
    switch(m_input.get_tensor().get_elem_size()) {
        ...
        case (sizeof(int16_t)):
            if (out_elem_size == sizeof(int8_t)) {
              ...
              mli_krn::mli_krn_rescale<int32_t, int32_t>(&m_tile_metadata.input,
                                                         &m_tile_metadata.in_bias,
                                                         &m_tile_metadata.scale,
                                                         &m_tile_metadata.shift,
                                                         &m_tile_metadata.out_bias,
        ...
        case (sizeof(int8_t)):
            if (out_elem_size == sizeof(int8_t)) {
              mli_krn::mli_krn_rescale<int8_t, int8_t>(&m_tile_metadata.input,
                                                        &m_tile_metadata.in_bias,
                                                        &m_tile_metadata.scale,
                                                        &m_tile_metadata.shift,
                                                        &m_tile_metadata.out_bias,
...
void Rescale::GetIOSizesAndOffsets(uint32_t& enc_param_size, uint32_t& inp_bias_offset, uint32_t& scale_offset,
  ...
  enc_param_size = m_enc_param.GetSubTensor().get_dim(0);
  inp_bias_offset = 0;
  scale_offset = m_tile_param_max_size * m_input.get_elem_size();
  shift_offset = m_tile_param_max_size * (m_input.get_elem_size() + sizeof(int16_t));
  out_bias_offset = m_tile_param_max_size * (m_input.get_elem_size() + sizeof(int16_t) + sizeof(int8_t));
}
