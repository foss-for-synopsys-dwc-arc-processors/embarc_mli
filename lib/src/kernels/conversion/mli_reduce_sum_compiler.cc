...
ReduceSum_CS::ReduceSum_CS(const lib_mli::PlatformDescription pd,
...
mli_status ReduceSum_CS::AttachBufferOffsets(const OffsetBuffer &input,
...
mli_status ReduceSum_CS::GetKernelPrivateData(void *kernel_private_data_buffer ) {
    ReduceSumPrivateData opaque_obj;
    ...
    for(int32_t i = 0; i < (int32_t)(opaque_obj.io_rank); i++) {
        if( i != opaque_obj.reduce_axis ){
            MLI_ASSERT(m_in.get_tensor().get_dim(i) == m_out.get_tensor().get_dim(i));
        }
        else{
            MLI_ASSERT(1 == m_out.get_tensor().get_dim(i));
        }
    }
...
unsigned ReduceSum_CS::GetKernelPrivateDataSize() const {
    return sizeof(ReduceSumPrivateData);
}

unsigned ReduceSum_CS::GetRuntimeObjectSize() const {
    return sizeof(ReduceSum);
}
