...
mli_status mli_krn_conv2d_hwc_sa8_sa8_sa32_k1x1(
        const mli_tensor * in, 
        const mli_tensor * weights, 
        const mli_tensor * bias, 
        const mli_conv2d_cfg * cfg, 
    ...
    pointwise_convolution2D_hwc_krnpad<int8_t, int8_t, int32_t, mli_acc32_t>(
                in_ftrs, wt, bs, out_ftrs, &cent_area, params,
                (int8_t)val_limit.min, (int8_t)val_limit.max, in_ch, in_width, in_height, 
                out_ch, out_width, out_height, kernel_height, kernel_width,
                stride_height, stride_width, padding_top, padding_left, padding_bot, padding_right);
...
mli_status mli_krn_conv2d_hwc_sa8_sa8_sa32(
    ...
    if ((kernel_h == 1) && (kernel_w == 1)) {
        return mli_krn_conv2d_hwc_sa8_sa8_sa32_k1x1(in, weights, bias, cfg, out);
    } else {
...
char * mli_debug_krn_conv2d_hwc_sa8_sa8_sa32(
    ...
    if ((kernel_h == 1) && (kernel_w == 1)) {
        return (char*)"mli_krn_conv2d_hwc_sa8_sa8_sa32_k1x1";
    } else {
