...
DepthwiseConv2d::DepthwiseConv2d(PrivateData* kernel_private_data_buffer,
  ...
  auto address = membases[private_data->m_metadata_buffer.get_mem_idx()] +
      private_data->m_metadata_buffer.get_offset();
  ...
  // TODO: Move partly or all into a helper function and use for each tensor
  {
    ...
    MLI_ASSERT(private_data->m_input_buffer.get_mem_idx() < uint32_t(num_mems));
    address = membases[private_data->m_input_buffer.get_mem_idx()] +
        private_data->m_input_buffer.get_offset();
    m_metadata->input = *new (reinterpret_cast<int8_t*>(address)) mli_tensor;
...

  {
    ...
    MLI_ASSERT(private_data->m_output_buffer.get_mem_idx() < uint32_t(num_mems));
    address = membases[private_data->m_output_buffer.get_mem_idx()] +
        private_data->m_output_buffer.get_offset();
    m_metadata->output = *new (reinterpret_cast<int8_t*>(address)) mli_tensor;
...

  {
    ...
    MLI_ASSERT(private_data->m_weights_buffer.get_mem_idx() < uint32_t(num_mems));
    address = membases[private_data->m_weights_buffer.get_mem_idx()] +
        private_data->m_weights_buffer.get_offset();
    m_metadata->weights = *new (reinterpret_cast<int8_t*>(address)) mli_tensor;
