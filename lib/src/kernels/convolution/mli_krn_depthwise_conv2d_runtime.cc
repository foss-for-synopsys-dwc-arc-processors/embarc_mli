...
DepthwiseConv2d::DepthwiseConv2d(void* kernel_private_data_buffer,
  ...
  m_metadata.weights = Tensor<InternalBuffer, 3>(private_data.weights, membases, num_mems);
...
mli_status DepthwiseConv2d::Issue() {
  ...
  if (i_elem_size == sizeof(int8_t) &&
    ...
    QTensor<InternalBuffer, 3> qweights{
    ...
    conv2d_prepare_and_run<int8_t, int8_t, int32_t, mli_8x8_accu_t, LAYOUT_HWC,
                           ::mli::CONV_DEPTHWISE, /* io_rank */ 4,
                           /* w_rank */ 3, DwConv2DConfig>(
        qinput, qweights, m_metadata.output, m_metadata.config);
  } else {
