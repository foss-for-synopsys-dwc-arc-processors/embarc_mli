...
//========================================================
template <typename i_T, typename w_T, typename o_T, typename acc_T, typename quant_T, int fix_kernel_width, int fix_kernel_height>
    ...
    for (int H_idx = row_begin; H_idx < row_end; H_idx++) {
        for (int W_idx = clmn_begin; W_idx < clmn_end; W_idx++) {
            ...
            for (int in_ch_idx = 0; in_ch_idx < in.ch; in_ch_idx++) {
                ...
                // Additiona calculations for padding areas only:
                // out_val += (sum(x_zp*w) - sum_i(x_zp*w_zp))
                //============================================
...
MLI_FORCE_INLINE void depthwise_convolution2D_wrapper(
    ...
    mli::krn::depthwise_convolution2D<i_T, w_T, o_T, acc_T, quant_T, fix_kernel_width, fix_kernel_height>(
            in_, weights_, out_, perception_area, quant_params,
...
//====================================================================================
template <typename i_T, typename w_T, typename o_T, typename acc_T, typename quant_T,
...
MLI_FORCE_INLINE void conv2d_prepare_and_run(
    ...
    if (conv_type == CONV_GENERAL) {
        ...
        depthwise_convolution2D_wrapper<i_T, w_T, o_T, acc_T, quant_T, fix_kernel_width, fix_kernel_height>(
