...
Add::Add(void* kernel_private_data_buffer, size_t size, uint64_t membases[], int num_mems) {
  MLI_ASSERT(size == sizeof(EltwisePrivateData));
  EltwisePrivateData private_buffer(kAddId);
  memcpy(&private_buffer, kernel_private_data_buffer, sizeof(EltwisePrivateData));
  MLI_ASSERT(private_buffer.kernel_id == kAddId);
...
mli_status Add::Issue() {
  if (m_i_elem_size == sizeof(int8_t) && m_o_elem_size == sizeof(int8_t)) {
    ::mli::krn::eltwise_prepare_and_run
      <int8_t, int8_t, ::mli::ELTWISE_ADD, false>(&m_tile_input_left, &m_tile_input_right, &m_tile_output);
  }
  else if (m_i_elem_size == sizeof(int16_t) && m_o_elem_size == sizeof(int16_t)) {
    ::mli::krn::eltwise_prepare_and_run
      <int16_t, int16_t, ::mli::ELTWISE_ADD, false>(&m_tile_input_left, &m_tile_input_right, &m_tile_output);
  }
  else if (m_i_elem_size == sizeof(int32_t) && m_o_elem_size == sizeof(int32_t)) {
    ::mli::krn::eltwise_prepare_and_run
      <int32_t, int32_t, ::mli::ELTWISE_ADD, false>(&m_tile_input_left, &m_tile_input_right, &m_tile_output);
  } else {
...
mli_status Add::Prefetch() {
  return MLI_STATUS_OK;
}

mli_status Add::Update() {
  m_input_right.Next();
  m_input_left.Next();
  m_output.Next();
...
void Add::GetIOSizesAndOffsets(uint32_t input_left_size[kEltwiseRank], uint32_t input_right_size[kEltwiseRank], uint32_t output_size[kEltwiseRank],
