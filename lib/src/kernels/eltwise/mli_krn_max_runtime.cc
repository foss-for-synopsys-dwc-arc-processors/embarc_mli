...
Max::Max(void* kernel_private_data_buffer, size_t size, uint64_t membases[], int num_mems) {
  MLI_ASSERT(size == sizeof(EltwisePrivateData));
  EltwisePrivateData private_buffer(kMaxId);
  memcpy(&private_buffer, kernel_private_data_buffer, sizeof(EltwisePrivateData));
  MLI_ASSERT(private_buffer.kernel_id == kMaxId);
...
mli_status Max::Issue() {
 if (m_i_elem_size == sizeof(int8_t) && m_o_elem_size == sizeof(int8_t)) {
    ::mli::krn::eltwise_prepare_and_run
      <int8_t, int8_t, ::mli::ELTWISE_MAX, false>(&m_input_left, &m_input_right, &m_output);
  }
  else if (m_i_elem_size == sizeof(int16_t) && m_o_elem_size == sizeof(int16_t)) {
    ::mli::krn::eltwise_prepare_and_run
      <int16_t, int16_t, ::mli::ELTWISE_MAX, false>(&m_input_left, &m_input_right, &m_output);
  }
...
mli_status Max::Prefetch() {
  return MLI_STATUS_OK;
}

mli_status Max::Update() {
  return MLI_STATUS_OK;
}
