...
Min_CS::Min_CS(const lib_mli::PlatformDescription pd,
...
unsigned Min_CS::GetKernelPrivateDataSize() const {
  return sizeof(EltwisePrivateData);
}

unsigned Min_CS::GetRuntimeObjectSize() const {
  return sizeof(Min);
}
...
mli_status Min_CS::AttachBufferOffsets(const Tensor<OffsetBuffer, kEltwiseRank> &input_left,
...
mli_status Min_CS::AttachBufferOffsets(const OffsetBuffer &input_left,
...
unsigned Min_CS::GetInputLeftBufferSize() {
  return m_in_left_buffer_size;
}
unsigned Min_CS::GetInputRightBufferSize() {
