...
Mul_CS::Mul_CS(const lib_mli::PlatformDescription pd,
  ...
  m_output_buffer_size
      = service::GetBufferSize(output.get_rank(), output_shape, output_stride);
}
Mul_CS::Mul_CS(const lib_mli::PlatformDescription pd,
...
unsigned Mul_CS::GetKernelPrivateDataSize() const {
  return sizeof(EltwisePrivateData);
}

unsigned Mul_CS::GetRuntimeObjectSize() const {
  return sizeof(Mul);
}

mli_status Mul_CS::GetKernelPrivateData(void* kernel_private_data_buffer) {
  EltwisePrivateData obj(kMulId);
...
mli_status Mul_CS::AttachBufferOffsets(const Tensor<OffsetBuffer, kEltwiseRank> &input_left,
...
mli_status Mul_CS::AttachBufferOffsets(const OffsetBuffer &input_left,
...
unsigned Mul_CS::GetInputLeftBufferSize() {
  return m_in_left_buffer_size;
}
unsigned Mul_CS::GetInputRightBufferSize() {
  return m_in_right_buffer_size;
}
unsigned Mul_CS::GetOutputBufferSize() {
