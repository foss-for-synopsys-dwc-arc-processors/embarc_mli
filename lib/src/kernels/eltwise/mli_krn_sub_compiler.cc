...
Sub_CS::Sub_CS(const lib_mli::PlatformDescription pd,
...
unsigned Sub_CS::GetKernelPrivateDataSize() const {
  return sizeof(EltwisePrivateData);
}

unsigned Sub_CS::GetRuntimeObjectSize() const {
  return sizeof(Sub);
}

mli_status Sub_CS::GetKernelPrivateData(void* kernel_private_data_buffer) {
  EltwisePrivateData obj(kSubId);
...
mli_status Sub_CS::AttachBufferOffsets(const Tensor<OffsetBuffer, kEltwiseRank> &input_left,
...
mli_status Sub_CS::AttachBufferOffsets(const OffsetBuffer &input_left,
...
unsigned Sub_CS::GetInputLeftBufferSize() {
  return m_in_left_buffer_size;
}
unsigned Sub_CS::GetInputRightBufferSize() {
  return m_in_right_buffer_size;
}
unsigned Sub_CS::GetOutputBufferSize() {
