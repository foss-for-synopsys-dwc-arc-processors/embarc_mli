...
typedef vNx4accint_t mli_fx8_accu_t;
#elif !defined(MLI_BUILD_REFERENCE) && defined(__FXAPI__)
typedef __v2i32_t mli_fx8_accu_t;
#else
...
mli_status mli_krn_avepool_hwc_fx8_k2x2(const mli_tensor * in, const mli_pool_cfg * cfg, mli_tensor * out) {
    ...
    mli::krn::mli_krn_avepool_hwc<int8_t, mli_fx8_accu_t, AVEPOOL_FIXED_KRN_SIZE_2>(in, cfg, out);
...
mli_status mli_krn_avepool_hwc_fx8_k3x3(const mli_tensor * in, const mli_pool_cfg * cfg, mli_tensor * out) {
    ...
    mli::krn::mli_krn_avepool_hwc<int8_t, mli_fx8_accu_t, AVEPOOL_FIXED_KRN_SIZE_3>(in, cfg, out);
    return MLI_STATUS_OK;
}
...
mli_status mli_krn_avepool_hwc_fx8(const mli_tensor * in, const mli_pool_cfg * cfg, mli_tensor * out) {
    ...
    if ((kernel_w == 3) && (kernel_h == 3)) {
        ...
        mli::krn::mli_krn_avepool_hwc<int8_t, mli_fx8_accu_t, AVEPOOL_NO_FIXED_KRN_SIZE>(in, cfg, out);
    }
