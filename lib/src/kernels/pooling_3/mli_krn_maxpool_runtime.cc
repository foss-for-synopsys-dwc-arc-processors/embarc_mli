...
MaxPool2D::MaxPool2D(PrivateData* kernel_private_data_buffer, size_t size,
  ...
  assert(maxpool2d_private_buffer->descr_mem_id < num_mems);
  int descr_mem_id = maxpool2d_private_buffer->descr_mem_id;
  uint32_t cfg_data_offset = maxpool2d_private_buffer->tensor_data_offset;
  m_cfg =
      new (reinterpret_cast<int8_t*>(membases[descr_mem_id] + cfg_data_offset))
          mli_pool_cfg;
  ...
  uint32_t input_tensor_data_offset = cfg_data_offset + sizeof(mli_pool_cfg);
  m_input = new (reinterpret_cast<int8_t*>(
      membases[input_mem_id] + input_tensor_data_offset)) mli_tensor;
  ...
  uint32_t output_tensor_data_offset =
      input_tensor_data_offset + sizeof(mli_tensor);
  m_output = new (reinterpret_cast<int8_t*>(membases[output_mem_id]) +
                  output_tensor_data_offset) mli_tensor;
...
mli_status MaxPool2D::Issue() {
  if (m_io_elem_size == sizeof(int16_t)) {
    ...
    for (uint32_t i = 0; i < m_batch_number; i++) {
      m_input->data.mem.pi16 += m_input_batch_offset * i;
      m_output->data.mem.pi16 += m_output_batch_offset * i;
      mli3_krn_maxpool_hwc_io16(m_input, m_cfg, m_output);
    }
    ...
    for (uint32_t i = 0; i < m_batch_number; i++) {
      m_input->data.mem.pi8 += m_input_batch_offset * i;
      m_output->data.mem.pi8 += m_output_batch_offset * i;
      mli3_krn_maxpool_hwc_io8(m_input, m_cfg, m_output);
    }
