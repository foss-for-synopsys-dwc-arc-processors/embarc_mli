        ...
        for (int pos1 = 0; pos1 < out_prv->shape[1]; pos1++) {
            for (int pos2 = 0; pos2 < out_prv->shape[2]; pos2++) {
                ...
                if (out_prv->shape[3] & 1) {
                    // TODO: Apply MLI_MATH_LAYER
                    v2accum40_t tmp_acc = fx_v2a40_mpy_nf_v2q15(sum_recip, mli_prv_load_1_sample(vec_out));
                    mli_prv_store_1_sample(vec_out, fx_v2q15_cast_nf_asl_rnd_v2a40(tmp_acc, shift));
                    vec_out += 1;
                }
                for (int pos3 = 0; pos3 < out_prv->shape[3] >> 1; pos3++) {
                    ...
                    mli_prv_store_2_samples(vec_out, fx_v2q15_cast_nf_asl_rnd_v2a40(tmp_acc, shift));
                    vec_out += 2;
                }
            }
        }
