...
mli_status mli_krn_sigm_fx8(const mli_tensor* in, mli_tensor* out) {
    ...
    mli_prv_activation_lut_fx8(
            (MLI_PTR(int8_t))in->data.mem.void_p, (MLI_OUT_PTR(int8_t))out->data.mem.void_p, &sigmoid_lut_fx16, in->el_params.fx.frac_bits,
...
mli_status mli_krn_sigm_fx16(const mli_tensor* in, mli_tensor* out) {
    ...
    mli_prv_activation_lut_fx16(
            (MLI_PTR(int16_t))in->data.mem.void_p, (MLI_OUT_PTR(int16_t))out->data.mem.void_p, &sigmoid_lut_fx16, in->el_params.fx.frac_bits,
            (int)mli_prv_count_elem_num(in));
    mli_prv_copy_tensor_format(in, out);
...
mli_status mli_krn_sigm_sa8(const mli_tensor* in, mli_tensor* out) {
    ...
    mli_prv_activation_lut_sa8(
            (MLI_PTR(int8_t))in->data.mem.void_p, (MLI_OUT_PTR(int8_t))out->data.mem.void_p, &sigmoid_lut_fx16, 
