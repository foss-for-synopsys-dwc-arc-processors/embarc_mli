...
static MLI_FORCE_INLINE void mli_prv_store_n_samples (MLI_OUT_PTR (int8_t) __restrict out, vNx4char_t data, pvNx4 predicate) {
    vvst(data, predicate, (signed char __vccm *)(out));
}

static MLI_FORCE_INLINE void mli_prv_store_n_samples (MLI_OUT_PTR (int16_t) __restrict out, vNx2short_t data, pvNx2 predicate) {
    vvst(data, predicate, (short __vccm *)(out));
}
...
static MLI_FORCE_INLINE void mli_prv_store_n_samples (MLI_OUT_PTR (int8_t) __restrict out, vNx4char_t data, int predicate_limit) {
	pvNx4 predicate = mli_prv_pvNx4_init(predicate_limit);
    vvst(data, predicate, (signed char __vccm *)(out));
}

static MLI_FORCE_INLINE void mli_prv_store_n_samples (MLI_OUT_PTR (int16_t) __restrict out, vNx4short_t data, int predicate_limit) {
	if (predicate_limit > _VDSP_NUM_16BIT_LANES) {
		...
		vvst(data.hi, predicate, (short __vccm *)(out));
	} else {
		pvNx2 predicate = mli_prv_pvNx2_init(predicate_limit);
		vvst(data.lo, predicate, (short __vccm *)(out));
	}
