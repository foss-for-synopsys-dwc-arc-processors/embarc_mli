...
int8_t* tensor_quantizer::tensor_get_data_ptr(
    ...
    if (tensor->rank > 0) {
        ...
        return (int8_t*) &(tensor->data.mem.i8);
    }
}
...
int16_t* tensor_quantizer::tensor_get_data_ptr(
    ...
    if (tensor->rank > 0) {
        ...
        return (int16_t*) &(tensor->data.mem.i16);
    }
}
...
int32_t* tensor_quantizer::tensor_get_data_ptr(
    ...
    if (tensor->rank > 0) {
        ...
        return (int32_t*) &(tensor->data.mem.i32);
    }
}
...
float* tensor_quantizer::tensor_get_data_ptr(
    ...
    if (tensor->rank > 0) {
        ...
        return (float*) &(tensor->data.mem.f32);
    }
}
...
void tensor_quantizer::quantize_float_data_routine(const float* src, uint32_t src_size, mli_tensor* dst) {
    ...
    for (int scale_idx = 0; scale_idx < scales_num; ++scale_idx) {
        ...
        const float scale_val = ldexp(1.f / (float)mli_hlp_tensor_scale(dst, scale_idx), scale_fraq_bits);
...
void tensor_quantizer::dequantize_tensor_data_routine(const mli_tensor* src, float* dst, uint32_t dst_size) {
    ...
    for (int scale_idx = 0; scale_idx < scales_num; ++scale_idx) {
        ...
        const float scale_val = ldexp((float)mli_hlp_tensor_scale(src, scale_idx), -scale_fraq_bits);
