...
void postprocess_phase(const reporter_full* reporter,
  ...
  reporter->evaluate_and_report_case(cur_test->descr, test_metics,
...
int main() {
  ...
  for (int i = 0; i < kTestsNum; ++i) {
    ...
    postprocess_phase(&reporter, cur_test, out_mem_offset);
  }
